...
# --- UI: TESTING ---
    ...
    st.title(f"{subj}: {grade}"); st.caption(f"Student: {st.session_state.p_student}"); st.divider()

    if st.session_state.phase == "familiarity":
        topic_label = current_set.get('topic') or current_set.get('section_title')
        st.header(topic_label); st.subheader("Is the student familiar with this topic?")
        c1, c2 = st.columns([1, 5])
        if c1.button("Yes"): st.session_state.phase = "content"; st.rerun()
        if c2.button("No"):
            record(subj, grade, topic_label, "Familiarity", "No"); st.session_state.perfect_score = False
        ...
        if subj == "Mathematics":
            if st.session_state.phase in ["content", "mastery_retry"]:
                ...
                c1, c2 = st.columns(2)
                if c1.button("✅ Correct"): record(subj, grade, current_set['topic'], lbl, "Correct"); st.session_state.mastery_count += 1; advance_logic()
                ...
                st.subheader(f"Sub-Q {st.session_state.sub_idx+1}"); st.write(sub['q']); display_img(sub.get('image'), w=400)
                c1, c2 = st.columns(2)
            ...
            if isinstance(imgs, list) and len(imgs) > 0:
                ...
                for i, url in enumerate(imgs):
                    b = display_img(url, return_bytes=True)
