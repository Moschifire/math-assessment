...
def create_detailed_pdf(data):
    ...
    pdf.set_font("helvetica", "B", 12); pdf.set_x(l_col + 15); pdf.cell(r_col, 8, "AI Plan Overview", ln=True)
    pdf.set_font("helvetica", "", 9); pdf.set_x(l_col + 15); pdf.multi_cell(r_col, 5, clean_txt(overview))
...
def create_parent_report_pdf(data):
    ...
    # Branding
    pdf.set_fill_color(77, 208, 225) # Gradely Cyan
    ...
    pdf.circle(35, 65, 12, 'F') # Avatar placeholder
...
def create_learning_path_pdf(data):
    ...
    if table:
        with pdf.table(width=270, line_height=7) as t:
            for row in table[:13]: # Header + 12 weeks
                r = t.row()
                for cell in row: r.cell(clean_txt(cell))
...
def advance_logic():
    ...
    if subj == "Mathematics":
        ...
        if st.session_state.sub_idx < len(sec_qs)-1: st.session_state.sub_idx += 1
...
if page == "Admin Dashboard":
    ...
    if not st.session_state.admin_authenticated:
        ...
        if res.data:
            ...
            with v1: st.subheader("Internal Logs"); st.text(row['results']); st.subheader("Notes"); st.write(row['feedback'])
            with v2: st.subheader("AI Analysis"); st.markdown(row['ai_plan'])
    ...
    if st.session_state.step == "setup":
        ...
        if s_subj:
            ...
            if s_curr: grades = list(ALL_DATA[s_subj][s_curr].keys()); s_grade = st.selectbox("Grade", grades)
        t_tutor, t_student = st.text_input("Tutor"), st.text_input("Student")
        if st.button("Begin"):
        ...
        if st.session_state.phase == "familiarity":
            ...
            st.header(topic_lbl); st.subheader("Is student familiar?")
            if st.button("Yes"): st.session_state.phase = "content"; st.rerun()
            if st.button("No"):
        ...
        if st.button("💾 Save"):
            ...
            supabase.table("assessment_results").insert(payload).execute(); st.success("Saved!")
        if st.button("🔄 New"): st.session_state.clear(); st.rerun()
