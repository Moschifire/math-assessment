...
if 'step' not in st.session_state:
    st.session_state.update({"step": "setup", "results": [], "set_idx": 0, "sub_idx": 0, "phase": "familiarity", "mastery_count": 0, "perfect_score": True, "bottleneck_active": False, "admin_authenticated": False})
...
if page == "Admin Dashboard":
    ...
    if not st.session_state.admin_authenticated:
        ...
        if res.data:
            ...
            sel_label = st.selectbox("Select Assessment to Process:", df['selector_label'].tolist())
            row = df[df['selector_label'] == sel_label].iloc[0].to_dict()
            
            # --- ADMIN ACTION BUTTONS ---
            col_act1, col_ai_gen, col_act2 = st.columns([1, 1, 1])
            
            # 1. AI Generation Button
            if col_ai_gen.button("✨ Generate/Update AI Learning Plan"):
                with st.spinner("AI is analyzing data..."):
                    new_plan = generate_ai_report(row['student'], row['subject'], row['grade'], row['curriculum'], row['results'], row['feedback'])
                    # Update database with the generated plan
                    supabase.table("assessment_results").update({"ai_plan": new_plan}).eq("id", row['id']).execute()
                    st.success("Plan generated and saved to database!")
                    st.rerun()

            # 2. PDF Download (Only if plan exists)
            if row.get('ai_plan'):
                pdf_data = create_pdf(row)
                if pdf_data: 
                    col_act1.download_button(label=f"📥 Download PDF Report", data=pdf_data, file_name=f"Report_{row['student']}.pdf", mime="application/pdf")
            else:
                col_act1.warning("Generate AI Plan first to enable PDF download.")
            
            st.divider()
            c1, c2 = st.columns([1, 2])
            with c1: 
                st.subheader("Raw Assessment Log"); st.text(row.get('results', ''))
                st.subheader("Tutor Feedback"); st.write(row.get('feedback', ''))
            with c2: 
                st.subheader("Current AI Learning Plan")
                if row.get('ai_plan'): st.markdown(row['ai_plan'])
                else: st.info("No AI plan has been generated for this record yet.")
    ...
    if st.session_state.step == "setup":
        ...
        if st.session_state.phase == "familiarity":
            ...
            if c2.button("No"):
                ...
                if st.session_state.bottleneck_active: st.session_state.step = "summary"
                    ...
                    if subj == "Mathematics": advance_logic()
                    else:
                        if st.session_state.set_idx < len(content) - 1: st.session_state.update({"set_idx": st.session_state.set_idx + 1, "sub_idx": 0, "phase": "familiarity"})
                        else: st.session_state.step = "summary"
                st.rerun()
            ...
            if subj == "Mathematics":
                if st.session_state.phase in ["content", "mastery_retry"]:
                    ...
                    if c1.button("❌ Incorrect"):
                        ...
                        if st.session_state.bottleneck_active: st.session_state.step = "summary"
                        elif st.session_state.phase == "mastery_retry": advance_logic()
                ...
                st.info(curr_set.get('instruction', ''))
                ...
                st.write(q['q'])
                opt_imgs = q.get('options_images') or q.get('images') or []
                if isinstance(opt_imgs, list) and opt_imgs:
                    cols = st.columns(len(opt_imgs))
                    for i, u in enumerate(opt_imgs):
                ...
                if c1.button("✅ Correct"):
                    record(subj, grade, curr_set.get('section_title', 'Section'), f"Q{st.session_state.sub_idx+1}", "Correct")
                    if st.session_state.sub_idx < len(curr_set['questions']) - 1: st.session_state.sub_idx += 1; st.rerun()
                    else: advance_logic()
                if c2.button("❌ Incorrect"):
                    record(subj, grade, curr_set.get('section_title', 'Section'), f"Q{st.session_state.sub_idx+1}", "Incorrect"); st.session_state.perfect_score = False
                    if st.session_state.bottleneck_active: st.session_state.step = "summary"
                    else:
                        if st.session_state.sub_idx < len(curr_set['questions']) - 1: st.session_state.sub_idx += 1; st.rerun()
                        else: advance_logic()
        ...
        st.title("Assessment Complete"); df = pd.DataFrame(st.session_state.results); st.table(df)
        obs = st.text_area("Final Tutor Observations")
        if st.button("💾 Save Results to Database"):
            ...
            try: 
                ...
                st.success("Results saved! Admin can now generate the plan in the Dashboard.")
